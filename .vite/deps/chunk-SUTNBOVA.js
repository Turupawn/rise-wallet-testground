...
// node_modules/rise-wallet/dist/core/Porto.js
var Porto_exports = {};
...
// node_modules/rise-wallet/node_modules/zustand/esm/middleware.mjs
...
var devtoolsImpl = (fn, devtoolsOptions = {}) => (set2, get2, api) => {
...
// node_modules/rise-wallet/node_modules/zustand/esm/vanilla.mjs
...
// node_modules/rise-wallet/dist/core/Chains.js
var Chains_exports = {};
...
function assertEip712Transaction(transaction) {
  const { chainId, to, from: from24, paymaster, paymasterInput } = transaction;
  ...
  if (from24 && !isAddress(from24))
    throw new InvalidAddressError({ address: from24 });
...
function serializeTransactionEIP712(transaction) {
  const { chainId, gas, nonce, to, from: from24, value, maxFeePerGas, maxPriorityFeePerGas, customSignature, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
...
function transactionToMessage(transaction) {
  const { gas, nonce, to, from: from24, value, maxFeePerGas, maxPriorityFeePerGas, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
  return {
    txType: 113n,
    from: BigInt(from24),
    to: to ? BigInt(to) : 0n,
...
function serializeTransactionDeposit(transaction) {
  ...
  const { sourceHash, data, from: from24, gas, isSystemTx, mint: mint2, to, value } = transaction;
  const serializedTransaction = [
    sourceHash,
    from24,
    to ?? "0x",
...
function assertTransactionDeposit(transaction) {
  const { from: from24, to } = transaction;
  if (from24 && !isAddress(from24))
    throw new InvalidAddressError({ address: from24 });
  if (to && !isAddress(to))
    throw new InvalidAddressError({ address: to });
...
// node_modules/rise-wallet/dist/core/internal/_generated/chains.js
var chains_exports = {};
__export(chains_exports, {
  riseTestnet: () => riseTestnet
});
...
var all = [
  riseTestnet,
  ...Object.values(chains_exports).filter((c) => c.id !== riseTestnet.id)
];
var anvil2 = anvil;
...
// node_modules/rise-wallet/dist/core/Dialog.js
var Dialog_exports = {};
...
// node_modules/rise-wallet/dist/core/internal/intersectionObserver.js
...
// node_modules/rise-wallet/dist/core/internal/logger.js
...
// node_modules/rise-wallet/dist/core/internal/userAgent.js
function isSafari() {
...
// node_modules/rise-wallet/dist/core/Messenger.js
...
// node_modules/rise-wallet/dist/core/internal/promise.js
...
// node_modules/rise-wallet/dist/core/internal/utils.js
...
// node_modules/rise-wallet/dist/core/react-native/environment.js
var environment;
var reactNative = {
  get environment() {
    if (!environment)
...
// node_modules/rise-wallet/dist/core/react-native/utils.js
...
function iframe(options = {}) {
  ...
  return from13({
    ...
    setup(parameters) {
      ...
      const root2 = document.createElement("dialog");
      root2.dataset.porto = "";
      root2.setAttribute("role", "dialog");
      root2.setAttribute("aria-closed", "true");
      root2.setAttribute("aria-label", "Porto Wallet");
      root2.setAttribute("hidden", "until-found");
      ...
      const activatePage = () => {
        if (!dialogActive)
          return;
        dialogActive = false;
        root2.removeEventListener("click", onBlur);
      ...
      const activateDialog = () => {
        if (dialogActive)
          return;
        dialogActive = true;
        root2.addEventListener("click", onBlur);
      ...
      const showDialog = () => {
        ...
        root2.removeAttribute("aria-closed");
        root2.showModal();
      };
      const hideDialog = () => {
        if (!visible)
          return;
        visible = false;
        root2.setAttribute("hidden", "true");
        root2.setAttribute("aria-closed", "true");
        root2.close();
        for (const sibling of root2.parentNode ? Array.from(root2.parentNode.children) : []) {
          if (sibling === root2)
            continue;
...
// node_modules/rise-wallet/dist/core/internal/mode.js
function from16(mode2) {
  return {
    ...mode2,
    setup: mode2.setup ?? (() => () => {
    })
...
// node_modules/rise-wallet/dist/core/internal/permissions.js
var Schema = Permissions;
...
// node_modules/rise-wallet/dist/core/internal/permissionsRequest.js
var Schema2 = Request;
...
// node_modules/rise-wallet/dist/core/internal/siwe.js
...
function from17(abiError, options = {}) {
...
function getExecuteError(e, parameters) {
  ...
  if (!error?.data)
    return e;
  if (error.data === getSelector3(from20("error FnSelectorNotRecognized()")))
...
// node_modules/rise-wallet/dist/viem/ContractActions.js
async function getEip712Domain3(client, parameters) {
  const { account = client.account } = parameters;
  const account_ = account ? from10(account) : void 0;
  if (!account_)
...
// node_modules/rise-wallet/dist/viem/internal/relayActions.js
...
function parseExecutionError(e, { calls } = {}) {
  ...
  const getAbiError = (error2) => {
    try {
      ...
      if (data === "0xd0d5039b")
        return from17("error Unauthorized()");
      return {
        inputs: [],
...
// node_modules/rise-wallet/dist/viem/RelayActions.js
...
async function prepareCalls2(client, parameters) {
  ...
  const authorizeKeys = (parameters.authorizeKeys ?? []).map((key2) => toRelay(key2, { feeTokens: tokens, orchestrator }));
  const feeToken = (() => {
  ...
  const args = {
    ...
    capabilities: {
      ...
      requiredFunds,
      revokeKeys: revokeKeys?.map((key2) => ({
        hash: key2.hash
      }))
    },
    chain,
...
async function prepareUpgradeAccount2(client, parameters) {
  ...
  const authorizeKeys = keys.map((key) => {
    ...
    return toRelay({ ...key, permissions }, { feeTokens: tokens, orchestrator });
  });
...
async function sendCalls(client, parameters) {
  ...
  const preCalls = await Promise.all((parameters.preCalls ?? []).map(async (pre) => {
    ...
    const signature2 = await sign2(key2, {
      address: null,
      payload: digest2,
      webAuthn
    });
...
async function upgradeAccount2(client, parameters) {
  ...
  const { context, signatures } = parameters;
  const account = from10(context.account);
...
function wrap(value) {
  ...
  const self = recoverAddress({
    ...
    signature: from5(value.authorization)
  });
...
// node_modules/rise-wallet/dist/core/internal/erc8010.js
...
// node_modules/rise-wallet/dist/core/internal/requiredFunds.js
function toRelay2(requiredFunds, options) {
  ...
  return requiredFunds.map((requiredFund) => {
    ...
    return {
      ...
      value: from8(requiredFund.value, interopToken.decimals)
    };
  });
}

// node_modules/rise-wallet/dist/core/internal/tokens.js
...
async function resolveFeeToken(client, parameters) {
  ...
  const feeToken = feeTokens?.find((feeToken2) => {
    if (!addressOrSymbol)
      return false;
    if (addressOrSymbol === "native" && feeToken2.address === zeroAddress)
      return true;
    if (validate2(addressOrSymbol) && isEqual(feeToken2.address, addressOrSymbol))
      return true;
...
// node_modules/rise-wallet/dist/core/internal/modes/relay.js
    ...
    actions: {
      ...
      async createAccount(parameters2) {
        ...
        const adminKey = !mock ? await createWebAuthnP256({
          ...
          rpId: keystoreHost,
          userId: from3(eoa.address)
        }) : createHeadlessWebAuthnP256();
        const sessionKey = await toKey2(permissions, {
          chainId: client.chain.id,
          feeTokens
        });
        const adminKeys = admins?.map((admin) => from9(admin));
        ...
        const signInWithEthereum_response = await (async () => {
          ...
          const signature = await sign3(eoa, {
      ...
      async getAccountVersion(parameters2) {
        ...
        const latest = await getEip712Domain3(client, {
          account: from10(accountImplementation)
        }).then((x) => x.version);
        const current = await getEip712Domain3(client, {
          account: address
      ...
      async grantAdmin(parameters2) {
        ...
        const { client } = internal;
        const authorizeKey = from9(parameters2.key, {
          chainId: client.chain.id
        });
      ...
      async grantPermissions(parameters2) {
        ...
        const { client } = internal;
        const feeTokens = await getTokens(client);
        const authorizeKey = await toKey2(permissions, {
          chainId: client.chain.id,
          feeTokens
        });
        if (!authorizeKey)
        ...
        const signature = await sign2(adminKey, {
          address: null,
          payload: digest
        });
      ...
      async loadAccounts(parameters2) {
        ...
        const { address, credentialId, webAuthnSignature } = await (async () => {
          ...
          const webAuthnSignature2 = await sign({
            challenge: digest,
            getFn: webAuthn?.getFn,
            rpId: keystoreHost
          });
          const response = webAuthnSignature2.raw.response;
          const address2 = toHex2(new Uint8Array(response.userHandle));
        ...
        if (authorizeKey) {
          ...
          const signature2 = await sign2(adminKey, {
            address: null,
            payload: digest2
          });
          await sendPreparedCalls2(client, {
        ...
        const signInWithEthereum_response = await (async () => {
          ...
          }
          {
            ...
            const signature2 = await sign3(account, {
      ...
      async prepareCalls(parameters2) {
        ...
        const requiredFunds = toRelay2(parameters2.requiredFunds ?? [], {
          tokens
        });
      ...
      async signPersonalMessage(parameters2) {
        ...
        const signature = await sign3(account, {
          key,
...
// node_modules/rise-wallet/dist/core/internal/modes/dialog.js
  ...
  return from16({
    actions: {
      ...
      async createAccount(parameters2) {
        ...
        const account = await (async () => {
          if (request.method === "wallet_connect") {
            ...
            const adminKeys = account2.capabilities?.admins?.map((admin) => from9(admin, { chainId: client.chain.id })).filter(Boolean);
      ...
      async grantAdmin(parameters2) {
        ...
        const key = from9(params.key);
        if (!key)
          throw new Error("no key found.");
        const feeToken = await resolveFeeToken2(internal, parameters2);
        const provider = getProvider(store);
      ...
      async grantPermissions(parameters2) {
        ...
        const key = await toKey2(permissions, {
          chainId: client.chain.id
        });
        if (!key)
          throw new Error("no key found.");
      ...
      async loadAccounts(parameters2) {
        ...
        const accounts = await (async () => {
          ...
          return Promise.all(accounts2.map(async (account) => {
            const adminKeys = account.capabilities?.admins?.map((key2) => from9(key2)).filter(Boolean);
      ...
      async sendCalls(parameters2) {
        ...
        if (key && key.role === "session") {
          ...
          try {
            ...
            const signature = await sign2(key, {
              address: null,
              payload: req.digest,
              wrap: false
            });
...
// node_modules/rise-wallet/dist/core/Transport.js
...
// node_modules/rise-wallet/dist/viem/RelayClient.js
...
// node_modules/rise-wallet/dist/core/internal/schema/request.js
...
// node_modules/rise-wallet/dist/core/internal/store.js
...
// node_modules/rise-wallet/dist/core/internal/urlString.js
...
// node_modules/rise-wallet/dist/core/internal/provider.js
function from22(parameters) {
      ...
      return withDedupe(async () => {
        ...
        switch (request.method) {
          ...
          case "wallet_connect": {
            ...
            const { createAccount: createAccount2, email, grantAdmins: admins, grantPermissions: permissions, selectAccount, signInWithEthereum } = capabilities ?? {};
            const internal = {
              client,
              config,
              request,
            ...
            const { accounts } = await (async () => {
              if (email || createAccount2) {
                const { label = void 0 } = typeof createAccount2 === "object" ? createAccount2 : {};
...
function getActivePermissions(keys, { address }) {
  return keys.map((key) => {
    ...
    if (key.expiry > 0 && key.expiry < BigInt(Math.floor(Date.now() / 1e3)))
      return void 0;
    try {
      return encode(Schema, fromKey(key, { address }));
    } catch {
      return void 0;
    }
  }).filter(Boolean);
...
// node_modules/rise-wallet/dist/core/Storage.js
var Storage_exports = {};
...
function idb() {
  const store = typeof indexedDB !== "undefined" ? createStore3("porto", "store") : void 0;
  return from23({
    async getItem(name) {
...
function create3(parameters = {}) {
  ...
  const provider = from22(internal);
  let destroy = mode2 !== null ? mode2.setup({
    internal
  }) : () => {
  };
  return {
...
//# sourceMappingURL=chunk-SUTNBOVA.js.map
